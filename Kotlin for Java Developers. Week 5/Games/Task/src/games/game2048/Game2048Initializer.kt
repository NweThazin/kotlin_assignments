...
object RandomGame2048Initializer : Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        val randomNum = generateRandomStartValue()
        val gameBoardImpl = board as GameBoardImpl
        val emptyCells = gameBoardImpl.gameBoardCells.filterValues { it == null }
        if (emptyCells.isNotEmpty()) {
            val firstKey = emptyCells.keys.toList().first()
            return Pair(firstKey, randomNum)
        }
        return null
    }
}